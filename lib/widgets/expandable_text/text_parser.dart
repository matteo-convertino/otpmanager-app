...
class TextSegment extends Equatable {
  ...
  TextSegment copyWith({String? text}) {
    return TextSegment(text ?? this.text, name, isHashtag, isMention, isUrl);
  }

  @override
  List<Object?> get props => [text, name, isHashtag, isMention, isUrl];
}
...
List<TextSegment> parseText(String? text) {
  ...
  for (var match in matches) {
    ...
    if (match.start > start) {
      if (segments.isNotEmpty && segments.last.isText) {
        segments.last = segments.last.copyWith(
  ...
  if (start < text.length) {
    if (segments.isNotEmpty && segments.last.isText) {
      segments.last = segments.last.copyWith(text: text.substring(start));
    } else {
